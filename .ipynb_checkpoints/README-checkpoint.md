...
config.json file explanation:
  ...
  - "noise": this refers to calibration with small pulses
    - "width": 5.0, initial guess for the gaussian width to fit the noise peak.
    - "width_frac": 0.8, fraction of the previous width used to define upper and lower boundaries for the fitting algorithm.
    - "bound_frac_height": 0.2, fraction of the height of the noise peak used to define upper and lower boundaries for the fitting algorithm. The best guess for the height is the value found with the scipy function find_peaks.
  - "spe": this refers to calibration with small pulses
    - "width": 11.0, initial guess for the gaussian width to fit the SPE peak.
    - "width_frac": 0.5, fraction of the previous width used to define upper and lower boundaries for the fitting algorithm.
    - "bound_frac_height": 0.2, fraction of the height of the SPE peak used to define upper and lower boundaries for the fitting algorithm. The best guess for the height is the value found with the scipy function find_peaks.
  - "dpe": this refers to calibration with small pulses
    - "height_bounds_low": 20, lower boundary for the height of the gaussian DPE peak for the fitting algorithm.
    - "height_bounds_low": 5e5, upper boundary for the height of the gaussian DPE peak for the fitting algorithm.
  - "multi_pe": this refers to calibration with small pulses: only useful is fit_models.calib_fit_model is used in the function data_calibration.fit_hist, otherwise not useful in the simplified model that we currently use fit_models.calib_fit_model_easy.
    - "a3": 900, initial guess for height of the gaussian representing the triple-photoelectron peak.
    - "a4": 800, initial guess for height of the gaussian representing the 4-photoelectron peak.
    - "a5": 700, initial guess for height of the gaussian representing the 5-photoelectron peak.
  - "exp": this refers to calibration with small pulses
    - "tau_low": 8, lower boundary for the exponential decay of the exponential component used as a proxy for all effects not well represented by a sum of gaussians.
    - "tau_high": 1000, same as before, but upper boundary.
    - "ae_guess": 9000, guess for the exponential height, but actually not used now.
  - "fit": this is used in all fits
    - "maxfev": 10000, 